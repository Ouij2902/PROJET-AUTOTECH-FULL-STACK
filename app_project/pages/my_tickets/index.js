...
const MyTicketsPage = ({showErrorMessage}) => {
        ...
        (async () => {
            ...
            if (!loaded) {
                ...
                try {
                    ...
                    showErrorMessage("Il y a eu une erreur lors de la récupération de vos tickets", e.response.data);
                }
    ...
    const handleTicketAvancementSort = async (event) => {
        ...
        const ticketSort = await sortAvancementTicket(myTickets);
    }
    const sortAvancementTicket = async (myTickets) => {
        try {
            const sortedData = myTickets.sort( (a,b) => {
    ...
    const handleTicketPrioritySort = async (event) => {
        event.preventDefault();
        const ticketSort = await sortPriorityTicket(myTickets);
    }
    const sortPriorityTicket = async (myTickets) => {
        try {
            const sortedData = myTickets.sort( (a,b) => {
            ...
            <Columns.Column className="is-8 is-offset-2 middle_page">
                <Columns>
                    <Columns.Column className="right">
                        <Heading className="is-3">Liste de vos tickets</Heading>
...
// On exporte la page
export default ProtectedRoute(MyTicketsPage);